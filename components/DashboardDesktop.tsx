...
const STATUS_PROGRESS: Record<OrderStatus, number> = { PENDING: 0.18, IN_TRANSIT: 0.62, DELIVERED: 1 };
...
export default function DashboardDesktop({ isAuthenticated, profile, orders, dashboard }: DashboardDesktopProps) {
  ...
  const orderSummary = (order: UserOrder) => {
    ...
    return `${first.quantity}× ${first.productName} · ${first.variant}${extra > 0 ? ` +${extra}` : ""}`;
  };
  ...
  const statusMeta = (status: OrderStatus) => {
    switch (status) {
      case "IN_TRANSIT":
        return {
          label: tAccount("statusInTransit"),
          pill: "border-blue-700/[.28] bg-blue-700/[.14] text-blue-700 dark:border-blue-400/[.28] dark:bg-blue-400/[.14] dark:text-blue-400",
          bar: "bg-[linear-gradient(90deg,#2563eb,#22d3ee)]",
        };
      case "DELIVERED":
        return {
          label: tAccount("statusDelivered"),
          pill: "border-slate-900/[.08] bg-slate-100 text-slate-500 dark:border-white/[.08] dark:bg-surface-3 dark:text-ink-muted",
          bar: "bg-slate-400 dark:bg-ink-muted",
        };
      default:
        return {
          label: tAccount("statusPending"),
          pill: "border-amber-600/[.25] bg-amber-500/[.12] text-amber-700 dark:border-amber-400/[.28] dark:bg-amber-400/[.12] dark:text-amber-400",
                    ...
                    {shownOrders.map((order) => {
                      ...
                      return (
                        <motion.div
                          ...
                          transition={hoverSpring}
                          className={`relative flex items-center gap-[18px] overflow-hidden rounded-[22px] border border-slate-900/[.07] bg-slate-50 px-5 py-[18px] dark:border-hairline dark:bg-surface-2 ${GLOW_HOVER}`}
                        >
                          <span className="flex h-11 w-11 flex-none items-center justify-center rounded-[15px] border border-slate-900/[.07] bg-slate-100 dark:border-hairline dark:bg-surface-3">
                            <OrderIcon
                              size={18}
                              strokeWidth={1.9}
                              className={done ? "text-slate-400 dark:text-ink-muted" : "text-blue-700 dark:text-blue-400"}
                            />
                          </span>
                          <Link href={`/profile/orders/${order.id}`} className="min-w-[150px] flex-[1_1_190px]">
                            <span className="block text-sm font-semibold tracking-[-.025em]">#{order.orderNumber}</span>
                          ...
                          <span className="min-w-0 flex-[1_1_200px]">
                            <span className="mb-2 flex items-center gap-2">
                              <span
                                className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold ${meta.pill}`}
                              >
                                {meta.label}
                              </span>
                          ...
                          <span className="min-w-[88px] text-right text-[15px] font-semibold tracking-[-.03em]">
